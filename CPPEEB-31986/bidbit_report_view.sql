spool CPPEEB-27742_Descriptions.out
set define off
set linesize 300
set pagesize 1000
set serveroutput on
PROMPT +
PROMPT The following should be run without error:
PROMPT 1. Create index
PROMPT 2. Create view
PROMPT 3. Grant view

CREATE INDEX cus_arbor.qvr_mapping_idx1 ON cus_arbor.qvr_mapping (component_id) NOLOGGING;

CREATE OR REPLACE VIEW cus_arbor.bidbit_report_view
AS
SELECT /*+ parallel (bid 8 8 )  */ 
    ...
    bidbit_package.get_geocode_description(c.cust_geocode)                         AS customer_location,
    c.cust_geocode                                                                 AS customer_geocode,
    bidbit_package.get_geocode_description(c.bill_geocode)                         AS billing_location,
    ...
    bidbit_package.get_geocode_description(s.service_geocode)                      AS service_a_location,
    s.service_geocode                                                              AS service_a_geocode,
    bidbit_package.get_geocode_description(s.b_service_geocode)                    AS service_b_location,
    s.b_service_geocode                                                            AS service_b_geocode,
    bid.bill_invoice_row                                                           AS bill_invoice_row,
    bidbit_package.get_charge_type_description(bid.type_code)                      AS charge_type,
    bid.component_Id                                                               AS component_Id,   
    NVL(NVL(bit.rev_split_charge_id, crs.child_charge_id), bid.subtype_code)       AS charge_id, 
    (SELECT d.description_text from descriptions d
    WHERE
        NVL(NVL(bit.rev_split_charge_id, crs.child_charge_id), bid.subtype_code) = d.description_code
        AND d.language_code  = 1)                                                                                     AS charge_description,
    bidbit_package.get_taxable_location(bid.type_code,NVL(NVL(bit.rev_split_charge_id, crs.child_charge_id), bid.subtype_code),bid.billing_level) AS charge_taxable_location,
    ...
    bidbit_package.get_prorate_description(bid.type_code,bid.prorate_code)         AS rc_proration,
    bidbit_package.get_bill_level_description(bid.billing_level)                   AS charge_billing_level,
    ...
    bidbit_package.get_geocode_description(NVL(bit.geocode,bid.geocode))           AS taxed_location,
  ...
  FROM bill_invoice bi
    ...
    INNER JOIN customer_id_acct_map ciam150 ON ciam150.account_no = c.account_no
...
ORDER BY bill_ref_no, bill_invoice_row, charge_id, tax_type_code, crs.revenue_percent;

GRANT SELECT ON cus_arbor.bidbit_report_view TO PUBLIC;
